...
def analyze_diet_logs(
    ...
    try:
        ...
        diet_logs.sort(
            key=lambda x: (
                datetime.strptime(x.get("created_at"), "%Y-%m-%dT%H:%M:%S.%fZ")
                if x.get("created_at")
                else datetime.min
            )
        )
        ...
        if num_logged_days > 0:
            analysis_results["average_daily_calories"] = (
                sum(day["calories"] for day in daily_intake.values()) / num_logged_days
            )
            analysis_results["average_daily_protein"] = (
                sum(day["protein"] for day in daily_intake.values()) / num_logged_days
            )
            analysis_results["average_daily_carbs"] = (
                sum(day["carbs"] for day in daily_intake.values()) / num_logged_days
            )
            analysis_results["average_daily_fats"] = (
                sum(day["fats"] for day in daily_intake.values()) / num_logged_days
            )

        # Add comparison to targets if provided
        if target_calories is not None:
            analysis_results["calories_vs_target"] = (
                analysis_results["average_daily_calories"] - target_calories
            )
        if target_protein is not None:
            analysis_results["protein_vs_target"] = (
                analysis_results["average_daily_protein"] - target_protein
            )
        if target_carbs is not None:
            analysis_results["carbs_vs_target"] = (
                analysis_results["average_daily_carbs"] - target_carbs
            )
        if target_fats is not None:
            analysis_results["fats_vs_target"] = (
