...
def analyze_workout_logs(workout_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        unique_dates = set(log.get("workout_date") for log in workout_logs if log.get("workout_date"))
    ...
    if unique_dates:
        dates = sorted([datetime.datetime.strptime(str(d), "%Y-%m-%d") for d in unique_dates])
        time_span = (dates[-1] - dates[0]).days if len(dates) > 1 else 0
        analysis_results["consistency"]["time_span_days"] = time_span
        if time_span > 0:
             analysis_results["consistency"]["workouts_per_day_average"] = len(unique_dates) / time_span if time_span > 0 else len(unique_dates)
 ...
 # Basic validation for list types to prevent errors
                ...
                if log.get("weight_per_set"):
                    exercise_progress[exercise_id]["weights"].extend([w for w in log["weight_per_set"] if w is not None])
                if log.get("reps_per_set"):
                    exercise_progress[exercise_id]["reps"].extend([r for r in log["reps_per_set"] if r is not None])
                if log.get("sets_completed") is not None:
                    exercise_progress[exercise_id]["sets"].append(log["sets_completed"])

        except Exception as e:
            raise ProcessingError(f"Error processing exercise data for log ID {log.get('id', 'N/A')}: {e}")
    ...
    perceived_exertions = [log.get("perceived_exertion") for log in workout_logs if log.get("perceived_exertion") is not None]
    if perceived_exertions:
        ...
        analysis_results["perceived_exertion"] = f"Average perceived exertion: {avg_pe:.1f}"
 ...
 # In a real app, you might use NLP here to summarize or extract keywords
        # In a real app, you might use NLP here to summarize or extract keywords
            analysis_results["notes_summary"] = notes[:3] # Just include the first 3 notes for brevity
        except Exception as e:
