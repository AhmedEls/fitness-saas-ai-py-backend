    ...
    try:
        ...
        if not isinstance(data, dict):
            return (
                jsonify(
                    {
                        "error": "Invalid data structure: Top level should be a dictionary with trainee_ids as keys"
                    }
                ),
                400,
            )
        ...
        for trainee_id, trainee_data in data.items():
            ...
            if not isinstance(trainee_data, dict):
                suggestions_by_trainee[trainee_id] = {
                    "error": "Invalid data structure for trainee"
                }
                continue

            if "workout_logs" not in trainee_data or "diet_logs" not in trainee_data:
                suggestions_by_trainee[trainee_id] = {
                    "error": "Missing 'workout_logs' or 'diet_logs'"
                }
                continue
            ...
            if not isinstance(workout_logs, list) or not all(
                isinstance(log, dict) for log in workout_logs
            ):
                suggestions_by_trainee[trainee_id] = {
                    "error": "Invalid data structure for 'workout_logs'"
                }
                continue

            # Validate diet_logs structure (assuming a list of dictionaries)
            if not isinstance(diet_logs, list) or not all(
                isinstance(log, dict) for log in diet_logs
            ):
                suggestions_by_trainee[trainee_id] = {
                    "error": "Invalid data structure for 'diet_logs'"
                }
                continue
            ...
            if any(
                "created_at" not in log or "trainee_id" not in log
                for log in workout_logs
            ):
                suggestions_by_trainee[trainee_id] = {
                    "error": "Missing required field in workout logs"
                }
                continue
            if any(
                "created_at" not in log or "trainee_id" not in log for log in diet_logs
            ):
                suggestions_by_trainee[trainee_id] = {
                    "error": "Missing required field in diet logs"
                }
                continue
